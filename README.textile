...
Let's upload a file now. We need to add a file field to a standard "create user" form. Your form must have the right enctype attribute to support file uploads, e.g. <code>$form->create('Users', array('type' => 'file'));</code>. Note that we omit the field's _file_name suffix here.
