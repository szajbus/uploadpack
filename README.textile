...
UploadPack is a set of classes that makes file uploads in CakePHP as easy as possible. It works with almost no configuration, but if you need more flexibility you can easily override default settings.

What's included:

h4. UploadBehavior

p(. Attach it to your model, it will detect any uploaded file and save it to disk. It can even automatically generate thumbnails of uploaded images.
  
h4. UploadHelper

p(. Use it in your views to display uploaded images or links to files in general.
...
Let's upload a file now. We need to add a file field to a standard "create user" form. Note that we omit the field's _file_name suffix here.

<pre><code>
  <?= $form->file('User.avatar') ?>
</code></pre>
...
<pre><code>
  ...
  <?= $upload->image($user, 'User.avatar') ?>
  <br/>
  And now it's thumbnail:
  <?= $upload->image($user, 'User.avatar', 'thumb') ?>
</code></pre>
...
h4. UploadHelper::url($data, $field, $style = 'original', $urlize = true)
...
h4. UploadHelper::image($data, $field, $style = 'original', $options = array())
...
* $field - name of a field, like this 'Modelname.fieldname' (would be 'User.avatar' here)
* $style - name of a thumbnail's style
* $options - array of HTML attributes passed to $html->image()
...
<pre><code>
  <?= $upload->image($data['User'][0], 'User.avatar', 'thumb') ?>
</code></pre>
...
Copyright (c) 2008 Michał Szajbe (http://codetunes.com) and netguru (http://netguru.pl), released under the MIT license.
